...
def example_triple_des():
	...
	#d = "La cryptographie est traditionnellement  utilisée pour dissimuler des messages aux yeux de certains utilisateurs. Cette utilisation a aujourd’hui un intérêt d’autant plus grand que les communications via internet circulent dans des réseaux dont on ne peut garantir la fiabilité et la confidentialité. Désormais, la cryptographie sert non seulement à préserver la confidentialité des données mais aussi à garantir leur intégrité et leur authenticité. Soit Alice et Bob, deux interlocuteurs qui veulent s’échanger des messages chiffrés. Il existe deux grandes familles de cryptographie, la cryptographie asymétrique et la crypto- graphie symétrique."
	d = "Triple DES test string, to be encrypted and decrypted 19"
